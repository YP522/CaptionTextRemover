...
def fill_by_content_aware_alternative(img_str,mask_str,inpaint_radius=32):
    """
    ...
    :param inpaint_radius: Radius of a circular neighborhood of each point inpainted that is considered
    ...
    dest = cv2.inpaint(img, mask, inpaint_radius, cv2.INPAINT_NS)
    cv2.imwrite(f"{u.dt_string}/result_without_text.png",dest)

def inpainting_with_border(img_str,thresh_val=0,max_val=150,size=2,inpaint_radius=11):
    """
    ...
    :param inpaint_radius: The radius of circular neighborhood of each point inpainted that is considered
    by the algorithm. Small values look less blurred, while larger values look more pixelated or
    ...
    # do inpainting
    result1 = cv2.inpaint(img, thresh, inpaint_radius, cv2.INPAINT_TELEA)
    result2 = cv2.inpaint(img, thresh, inpaint_radius, cv2.INPAINT_NS)
